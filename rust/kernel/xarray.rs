...
impl<'a, T: PointerWrapper> Guard<'a, T> {
    ...
    ///
    /// Returns a `T::Borrowed` type for the owned `PointerWrapper` type.
...
// Convenience impl for `PointerWrapper` types whose `Borrowed`
...
impl<T: PointerWrapper> XArray<T> {
    ...
    pub fn alloc_limits(&self, value: Option<T>, min: u32, max: u32) -> Result<usize> {
        ...
        if ret < 0 {
            ...
            if !new.is_null() {
                // SAFETY: If `new` is not NULL, it came from the `PointerWrapper` we got
                // from the caller.
