...
static int apple_mbox_init_of(struct device *dev, struct apple_mbox *mbox,
			      struct device_node *node)
{
	...
	match = of_match_node(apple_mbox_of_match, node);
	if (!match)
		return -EINVAL;
	if (!match->data)
		return -EINVAL;

	mbox->regs = of_iomap(node, 0);
	if (IS_ERR(mbox->regs))
		return PTR_ERR(mbox->regs);

	mbox->hw = match->data;
	mbox->irq_recv_not_empty = of_irq_get_byname(node, "recv-not-empty");
	if (mbox->irq_recv_not_empty < 0)
		return -ENODEV;

	mbox->irq_send_empty = of_irq_get_byname(node, "send-empty");
	...
	ret = request_threaded_irq(
		mbox->irq_recv_not_empty, NULL, apple_mbox_recv_irq,
		IRQF_NO_AUTOEN | IRQF_ONESHOT | IRQF_NO_SUSPEND, irqname, mbox);
	if (ret)
		return ret;
	...
	ret = request_irq(mbox->irq_send_empty, apple_mbox_send_empty_irq,
...
int apple_mbox_init(struct device *dev, struct apple_mbox *mbox, int index)
{
	...
	ret = apple_mbox_init_of(dev, mbox, args.np);
	of_node_put(args.np);

	return ret;
}
EXPORT_SYMBOL(apple_mbox_init);
			   ...
			   const char *name)
{
	...
	return apple_mbox_init(dev, mbox, index);
}
EXPORT_SYMBOL(apple_mbox_init_byname);

void apple_mbox_shutdown(struct apple_mbox *mbox)
{
	apple_mbox_stop(mbox);
	iounmap(mbox->regs);
	free_irq(mbox->irq_recv_not_empty, mbox);
	free_irq(mbox->irq_send_empty, mbox);
}
