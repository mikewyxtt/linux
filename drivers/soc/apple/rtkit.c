...
{
	struct apple_rtkit *rtk = container_of(mbox, struct apple_rtkit, mbox);
					    ...
					    const struct apple_rtkit_ops *ops)
{
	...
	if (mbox_name)
		ret = apple_mbox_init_byname(dev, &rtk->mbox, mbox_name);
	else
		ret = apple_mbox_init(dev, &rtk->mbox, mbox_idx);
