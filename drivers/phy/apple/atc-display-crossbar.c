...
/*
 * Apple Silicon Type-C PHY Display Crossbar multiplexer driver
 *
...
static int apple_dpxbar_set(struct mux_control *mux, int state)
{
	...
	if (enable) {
		...
		/*
		 * This workaround was observed by tracing XNU's memory access
		 * and seems to work around some HW quirk:
...
MODULE_DESCRIPTION("Apple Silicon Type-C PHY display crossbar driver");
