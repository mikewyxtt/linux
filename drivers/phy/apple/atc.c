...
#define clkhw_to_apple_atcphy(_hw) container_of(_hw, struct apple_atcphy, clk)

#define genpd_to_apple_atcphy(_genpd) \
	container_of(_genpd, struct apple_atcphy, genpd)
...
static int atcphy_cio_power_off(struct apple_atcphy *atcphy)
{
	...
	dev_err(atcphy->dev, "cio power off now\n");
...
static int atcphy_usb3_power_on(struct phy *phy)
{
	...
	if (atcphy->mode != atcphy->target_mode) {
		dev_err(atcphy->dev,
			"ATCPHY did not come up; won't allow dwc3 to come up.\n");
		mutex_unlock(&atcphy->lock);
		return -EINVAL;
	}
...
static int atcphy_usb4_init(struct phy *phy)
{
	struct apple_atcphy *atcphy = phy_get_drvdata(phy);

	mutex_lock(&atcphy->lock);
	atcphy->nhi_available = true;
	mutex_unlock(&atcphy->lock);

	return 0;
}

static int atcphy_usb4_exit(struct phy *phy)
{
	struct apple_atcphy *atcphy = phy_get_drvdata(phy);

	mutex_lock(&atcphy->lock);
	atcphy->nhi_available = false;
	mutex_unlock(&atcphy->lock);
...
static const struct phy_ops apple_atc_usb4_phy_ops = {
	...
	.init = atcphy_usb4_init,
	.exit = atcphy_usb4_exit,
};
...
static int atcphy_wait_nhi(struct apple_atcphy *atcphy)
{
	if (!atcphy->nhi_available)
		return 0;
	if (atcphy->nhi_online)
		return 0;
	
	reinit_completion(&atcphy->nhi_online_event);
	mutex_unlock(&atcphy->lock);
	wait_for_completion_timeout(&atcphy->nhi_online_event,
				    msecs_to_jiffies(2000));
	mutex_lock(&atcphy->lock);
	WARN_ON(!atcphy->nhi_online);
...
static void atcphy_mux_set_work(struct work_struct *work)
{
	struct apple_atcphy *atcphy =
		container_of(work, struct apple_atcphy, mux_set_work);
...
static void atcphy_detach_genpd(struct apple_atcphy *atcphy)
{
	int i;
...
static int atcphy_probe(struct platform_device *pdev)
{
	...
	init_completion(&atcphy->nhi_shutdown_event);
	init_completion(&atcphy->nhi_online_event);
	...
	if (IS_ERR(atcphy->regs.pmgr))
		return PTR_ERR(atcphy->regs.pmgr);
